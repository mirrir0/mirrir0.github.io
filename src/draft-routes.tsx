    ...
    <article className="group relative">
      ...
      <Link to={`/draft/${draft.slug}`} className="block pr-10 no-underline text-inherit">
        <div className="flex flex-col md:flex-row md:items-baseline gap-1 md:gap-4 mb-1">
          <time className="text-muted-foreground font-mono text-sm shrink-0">{formatDate(draft.date)}</time>
          <h2 className="text-foreground group-hover:text-emerald-400 transition-colors font-mono">{draft.title}</h2>
        </div>
        {draft.description && (
          <p className="text-muted-foreground text-sm md:ml-[88px]">{draft.description}</p>
        )}
      </Link>
      {draft.tags?.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2 md:ml-[88px]">
          {draft.tags.map((tag: string) => (
            <span key={tag} className="text-xs font-mono px-2 py-1 bg-card text-muted-foreground rounded">{tag}</span>
          ))}
        </div>
      )}
    </article>
  ...
  return (
    <div className="py-8 md:py-12">
      <header className="mb-8 md:mb-12">
        <h1 className="text-2xl font-display text-foreground mb-2 tracking-wide">drafts</h1>
        <p className="text-muted-foreground font-mono text-sm">unpublished posts and works in progress</p>
      </header>
      ...
      <form onSubmit={(e) => { e.preventDefault(); create(); }} className="mb-8 md:mb-12">
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 px-4 py-2 bg-secondary border border-border rounded text-foreground font-mono text-sm placeholder:text-muted-foreground focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
          />
          <button
            type="submit"
            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-foreground font-mono text-sm rounded transition-colors"
          >
            + New Draft
          </button>
        </div>
      ...
      {drafts.length === 0 ? (
        <p className="text-muted-foreground font-mono text-sm">No drafts yet. Create one above to get started.</p>
      ) : (
      ...
      <footer className="mt-12 md:mt-16 pt-8 border-t border-border flex gap-6">
        <Link to="/" className="text-muted-foreground hover:text-card-foreground font-mono text-sm no-underline">&larr; back to home</Link>
        <Link to="/blog" className="text-muted-foreground hover:text-card-foreground font-mono text-sm no-underline">view published</Link>
      </footer>
    </div>
  );
}
  ...
  return (
    <div className="py-8 md:py-12">
      ...
      <footer className="mt-12 md:mt-16 pt-8 border-t border-border">
        <Link to="/drafts" className="text-muted-foreground hover:text-card-foreground font-mono text-sm no-underline">
          &larr; back to drafts
        </Link>
      </footer>
    </div>
